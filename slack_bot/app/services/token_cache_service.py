...
class TokenCacheService:
    """
    Service for caching and retrieving JWT tokens for Slack users.
    ...
    def get_token(
        ...
        """
        Get a valid JWT token for a Slack user.
        
        Returns:
            Tuple of (access_token, error_message)
        """
        # Check DB cache first
        cached = cls._get_from_cache(slack_user_id)
        if cached:
            logger.debug(f"Token cache hit for Slack user {slack_user_id}")
            return cached["access_token"], None
        
        # Fetch from game server (synchronous for Slack Bolt)
        logger.debug(f"Token cache miss for Slack user {slack_user_id}")
        return cls._fetch_from_game_server(slack_user_id)
    
    @classmethod
    @requires_flask_db
    def get_cached_player_name(cls, slack_user_id: str) -> Optional[str]:
    ...
    @classmethod
    @requires_flask_db
    def invalidate(cls, slack_user_id: str) -> None:
        """Remove a user's token from cache."""
        cached = db.session.get(TokenCache, slack_user_id)
        if cached:
            db.session.delete(cached)
            db.session.commit()
            logger.debug(f"Invalidated token cache for Slack user {slack_user_id}")
    ...
    @classmethod
    @requires_flask_db
    def _get_from_cache(cls, slack_user_id: str) -> Optional[Dict]:
        ...
        if cached.is_expired():
            logger.debug(f"Cached token expired for Slack user {slack_user_id}")
            db.session.delete(cached)
            db.session.commit()
            return None
    ...
    def _save_to_cache(
        ...
        existing = db.session.get(TokenCache, slack_user_id)
    ...
    @classmethod
    def _fetch_from_game_server(
        cls,
        slack_user_id: str,
    ) -> Tuple[Optional[str], Optional[str]]:
        """Fetch token from game server (synchronous)."""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Token cached for Slack user {slack_user_id} "
                ...
                logger.error(f"Game server error: {response.status_code} - {error_text}")
                return None, f"Game server error: {response.status_code}"
