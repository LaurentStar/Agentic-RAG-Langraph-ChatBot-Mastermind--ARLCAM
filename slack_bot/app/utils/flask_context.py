...
WHY THIS EXISTS:
Slack bot commands run in a separate context from Flask's request/response cycle.
Flask-SQLAlchemy requires an "app context" to access the database. 
...
def requires_flask_db(func: Callable[..., T]) -> Callable[..., T]:
    ...
    - Runs outside Flask's request cycle (e.g., Slack commands)
