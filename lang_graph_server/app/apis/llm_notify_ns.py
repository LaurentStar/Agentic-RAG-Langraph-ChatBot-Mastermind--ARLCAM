...
# ---------------------- Name Spaces ---------------------- #
user_commands_ns = Namespace('llm-monitoring', description='Update that will be given to all llms players (chat messages, player action, updates from master agent). T')


# ---------------------- Resources ---------------------- #
class TestPrompts(Resource):
    @user_commands_ns.expect(test, validate=True)
    def post(self):
...
# ---------------------- Building Resources ---------------------- #
user_commands_ns.add_resource(TestPrompts, '/test-tone-extract')
