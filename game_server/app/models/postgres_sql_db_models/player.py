...
class Player(db.Model):
    ...
    debt                                : Mapped[int] = mapped_column(unique=False, default=0) 
...
class ToBeInitiatedUpgradeDetails(db.Model):
    ...
    assassination_priority      : Mapped[CardType] = mapped_column(unique=False, create_type=False) 
    kleptomania_steal           : Mapped[bool] = mapped_column(unique=False) # Steals regardless of being blocked
    trigger_identity_crisis     : Mapped[bool] = mapped_column(unique=False) # Causes 1 other player to shuffle deck too
    identify_as_tax_liabity     : Mapped[bool] = mapped_column(unique=False) # Determine rather a target will be taxed or not
    tax_debt                    : Mapped[int] = mapped_column(unique=False) # The amount the target wil owe range [0-3] coins
